document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initialize dark mode only if toggle exists
  if (darkModeToggle) {
  ...
  function displayFilteredActivities() {
    ...
    Object.entries(allActivities).forEach(([name, details]) => {
      ...
      if (currentDifficulty) {
        ...
        // If "All" is selected, only show activities with no difficulty level
  ...
  function renderActivityCard(name, details) {
    ...
    // Create difficulty badge if difficulty level is set
    const difficultyBadgeHtml = details.difficulty_level ? `
      <span class="difficulty-badge" data-difficulty="${details.difficulty_level}">
  ...
  function handleShare(event) {
    ...
    // Create share text
    const shareText = `Check out ${activity} at Mergington High School! ${description} Schedule: ${schedule}`;
    ...
    if (button.classList.contains("share-facebook")) {
      ...
      // Email share
      const subject = `Activity: ${activity}`;
